#!/bin/bash
# One-shot launcher for the G1 IK-reach-to-okra PoC (ACT-independent).
#
#   bash start_ik_reach.sh            # DRY-RUN (no motion) — overlay view + click check
#   bash start_ik_reach.sh --live     # LIVE: arm WILL move (keep e-stop, clear area)
#
...
#   [4] Runs the dimos IK-reach app in this terminal (Ctrl-C stops everything)
#
...
if [ "$LIVE" = "--live" ]; then
  ...
  echo "*** LIVE MODE: the arm WILL move via rt/arm_sdk. Keep the e-stop in hand, area clear. ***"
else
  unset IK_REACH_LIVE 2>/dev/null || true
  echo "  (DRY-RUN: no motion. Pass --live to drive the arm.)"
fi

LOGFILE="${IK_REACH_LOG:-/tmp/ik_reach_app.log}"
...
echo "== [4/4] native dimos-viewer auto-opens in ~12s; starting dimos IK-reach app (Ctrl-C to stop) =="
( sleep 12; "$REPO/.venv/bin/dimos-viewer" \
    --connect rerun+http://127.0.0.1:9877/proxy --ws-url ws://127.0.0.1:3030/ws >/dev/null 2>&1 ) &
"$REPO/.venv/bin/dimos" run unitree-g1-ik-reach 2>&1 | tee -a "$LOGFILE"
