...
| `ollama_vlm.py` | `verify_harvest` via a **local Ollama vision model** (`make_ollama_verify`). Default **`moondream`** = caption + keyword match (~1s on Jetson; moondream returns empty to direct yes/no). `qwen3-vl:2b` = chat yes/no (~5s, multilingual). Auto-selected by model name. Fail-safe (no frame / Ollama down → False). |
