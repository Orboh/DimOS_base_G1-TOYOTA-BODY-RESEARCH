...
On first run the Rerun window may stay black briefly while the scene downloads from LFS.

```bash
uv pip install 'dimos[base,unitree,sim]'
dimos --simulation run unitree-g1-sim          # humanoid
dimos --simulation run unitree-g1-agentic-sim  # + agent + MCP server
```

### Real robot (Unitree G1 over WebRTC)
...
Do not skip the platform guide - latency, time sync, and safety habits matter: [Unitree G1](/docs/platforms/humanoid/g1/index.md).
...
| `dimos run unitree-g1-okra-ik-only-grasp-zed` | Okra harvesting - ZED + YOLO detection |
...
```bash
dimos run unitree-g1-agentic --daemon    # background
dimos status
dimos log -f
