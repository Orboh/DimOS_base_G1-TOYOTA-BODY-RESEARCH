...
import numpy as np
from sim_harvest_skills import _Q_CLOSE, SimHarvestSkills
...
def _parse_calib(log_path: str) -> list[tuple[int, tuple[float, float, float]]]:
    ...
    for m in re.finditer(
        r"\[calib\] Okra_(\d+) torso=\((-?[0-9.]+), (-?[0-9.]+), (-?[0-9.]+)\)", txt
    ):
...
class WalkHarvestSkills(SimHarvestSkills):
    ...
    def __init__(
        ...
        *,
        ...
        if not calib:
            raise RuntimeError(
                "bridge ログに [calib] Okra が無い（SIM_GRASP_FRICTION=1 で起動したか）"
            )
        ...
        print(
            f"[walk-skills] 歩行モード skills 起動 pick対象={sorted(i for i, _ in calib)} spawn={self._spawn}",
            flush=True,
        )
    ...
    def _base_pose(self) -> tuple[float, float, float]:
        ...
        m = re.findall(
            r"base=\((\+?-?[0-9.]+),(\+?-?[0-9.]+),[0-9.]+\) yaw=(\+?-?[0-9.]+)", self._blog._tail()
        )
        if m:
    ...
    def detect_okra(self) -> list[Okra]:
        ...
        out = []
        for k in sorted(
            self._okra, key=lambda kk: float(self._okra[kk][1][1]), reverse=not order_right
        ):
            ...
            out.append(
                Okra(
                    id=k,
                    pos_3d={"x": float(-p[1]), "y": float(p[0]), "z": float(p[2])},
                    ripeness=1.0,
                    reachable=True,
                )
            )
    ...
    def relative_move(self, lateral: float, forward: float = 0.0, yaw: float = 0.0) -> None:
        ...
        print(
            f"[walk-skills] relative_move(lat={lateral:+.2f},fwd={forward:+.2f}) → base={self._base_xy()}",
            flush=True,
        )
    ...
    def _align_to(self, oid: str, max_moves: int | None = None) -> np.ndarray:
        ...
        for _ in range(max_moves):
            ...
            dy = (
                0.0 if _SWEET_Y[0] <= p[1] <= _SWEET_Y[1] else (-0.3 if p[1] < _SWEET_Y[0] else 0.3)
            )
            if dx == 0.0 and dy == 0.0:
                print(
                    f"[walk-skills]   位置合わせ完了 okra_now={tuple(round(float(v), 3) for v in p)}",
                    flush=True,
                )
                return p
        ...
        p = self._okra_now(oid)
        print(
            f"[walk-skills]   位置合わせ打ち切り okra_now={tuple(round(float(v), 3) for v in p)}",
            flush=True,
        )
        return p
    ...
    def grasp_okra(self, okra: Okra, force: float) -> None:
        ...
        if _zl is not None and _zl <= _FALLEN_Z:
            print(
                f"[walk-skills] okra{okra.id} 倒れ検知 (z={_zl:.3f}≤{_FALLEN_Z}) → 把持スキップ（取れない）",
                flush=True,
            )
        ...
        if arm_after_pin:
            ...
            down = np.array(
                [
                    float(os.getenv("SIM_WALK_ARM_DOWN_X", "0.18")),
                    -0.16,
            ...
            print(
                f"[walk-skills] GRASP okra{okra.id}: 位置合わせ歩行（腕下げ・挙手はピン後）",
                flush=True,
            )
        else:
        ...
        try:
            ...
            for it in range(int(os.getenv("SIM_WALK_SERVO_ITERS", "6" if two_stage else "3"))):
                ...
                if d is None:
                    break
                print(
                    f"[walk-skills]   servo{it}({'z' if descend else 'xy'}): "
                    f"Δ={tuple(round(v, 3) for v in d)}",
                    flush=True,
                )
                ...
                tgt[2] = float(
                    np.clip(tgt[2], float(os.getenv("SIM_WALK_SERVO_Z_MIN", "-0.12")), 0.16)
                )
                r = self._ik.solve(tgt, [0.0] * 29)
                if r is None:
                    break
            ...
            try:
                self._ramp(
                    self._cur_arm, 2.5, grip_q=_Q_CLOSE
                )  # close は緩やかに（急閉じで剛体オクラを弾き出さない）
            ...
            if r_l is not None:
                self._ramp(
                    list(r_l.arm14), float(os.getenv("SIM_WALK_LIFT_RAMP", "2.0")), grip_q=_Q_CLOSE
                )
            self._hold(0.8, grip_q=_Q_CLOSE)
            okz = self._blog.okra_z_max()
            self._last_grasp_ok = okz > 0.82
            print(
                f"[walk-skills]   lift 後 okra_z(max)={okz:.3f} → {'✅把持' if self._last_grasp_ok else '❌未把持'}",
                flush=True,
            )
        finally:
