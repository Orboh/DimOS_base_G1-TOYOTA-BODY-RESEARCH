...
from dimos.robot.unitree.g1.harvest.grasp_sequence import _Q_BLADE_MAX, GraspSequence
...
def check_file_estop() -> tuple[bool, str]:
    ...
    mon.step()
    tripped = (
        mon.gate.is_paused() and (mon.gate.checkpoint(timeout=0) is False) and len(paused) == 1
    )
    ...
    return (
        ok,
        f"init_clear={init_clear} tripped={tripped} stop_said={stop_said} cleared={cleared} resume_said={resume_said}",
    )
...
def check_bladeguard() -> tuple[bool, str]:
    ...
    return (
        ok,
        f"commanded={over} published={captured[-1] if captured else None} (上限={_Q_BLADE_MAX})",
    )
