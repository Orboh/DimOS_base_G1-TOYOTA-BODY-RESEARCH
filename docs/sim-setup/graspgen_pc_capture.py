...
def main() -> int:
    ...
    _t = np.array(
        UsdGeom.Xformable(stage.GetPrimAtPath(target_path)).ComputeLocalToWorldTransform(
    ...
    if target_id is None:
        ...
        print(
            f"[capture] WARN: {target_path} が idToLabels に見つからず。unique ids={uniq}",
            flush=True,
        )
        return 1
    ...
    if n_px < 20:
        print(
            "[capture] ERROR: マスク画素が少なすぎる（カメラが対象を捉えていない可能性）",
            flush=True,
        )
    ...
    if rgba is not None:
        cv2.imwrite(
            ...
            cv2.cvtColor(np.asarray(rgba)[..., :3], cv2.COLOR_RGB2BGR),
        )
