...
RIGHT7_REACH = [float(x) for x in os.getenv(
    "PROBE_RIGHT7", "0.1926,0.0107,0.0219,-0.1247,-0.0029,-0.0889,0.0064").split(",")]
...
def main() -> int:
    ...
    import omni.usd
    from pxr import Gf, Usd, UsdGeom, UsdPhysics, UsdShade, PhysxSchema, PhysicsSchemaTools
    ...
    print(f"[probe] num_dof={robot.num_dof} gripper(非canon)={[(i, dof_names[i]) for i in gripper_idx]}",
          flush=True)
    ...
    for gi in gripper_idx:
        print(f"[probe] gripper dof[{gi}] {dof_names[gi]} limit=({dof_lo[gi]:.4f},{dof_hi[gi]:.4f})", flush=True)
    ...
    if os.getenv("PROBE_INSPECT", "0") == "1":
        ...
        for p in rhb:
            nm = p.split("/")[-1]
            d = pclose[p] - popen[p]
            print(f"[inspect]  {nm:<28} open={np.round(popen[p],4).tolist()} "
                  f"→close={np.round(pclose[p],4).tolist()} 移動={np.linalg.norm(d)*1000:.1f}mm", flush=True)
        ...
        if len(moved) >= 2:
            ...
            print(f"[inspect] 可動2指={a.split('/')[-1]},{b.split('/')[-1]} "
                  f"開隙間={sep_open*1000:.1f}mm → 閉隙間={sep_close*1000:.1f}mm "
                  f"(この間の径の物体しか挟めない)", flush=True)
    ...
    if hand_base is None or not finger1 or not finger2:
        print(f"[probe] ERROR: 指リンク検出失敗 base={hand_base} f1={finger1} f2={finger2}", flush=True)
        sim_app.close()
        return 1
    ...
    print(f"[probe] 指先ペア tip1={tip1.split('/')[-1]}@{np.round(t1,4).tolist()} "
          f"tip2={tip2.split('/')[-1]}@{np.round(t2,4).tolist()}", flush=True)
    print(f"[probe] gap_center={np.round(gap_center,4).tolist()} "
          f"squeeze_axis={np.round(saxis,3).tolist()} 開隙間={np.linalg.norm(squeeze_axis)*1000:.1f}mm "
          f"(水平成分={np.hypot(saxis[0],saxis[1]):.2f} / 鉛直成分={abs(saxis[2]):.2f})", flush=True)
    ...
    def bind_gripper_material(gm):
        for p in Usd.PrimRange(stage.GetPrimAtPath("/G1")):
            if p.HasAPI(UsdPhysics.CollisionAPI) and "right_hand" in p.GetName().lower():
                UsdShade.MaterialBindingAPI.Apply(p).Bind(gm, UsdShade.Tokens.weakerThanDescendants, "physics")
    ...
    def spawn_object(diam_m, gm):
        ...
        UsdShade.MaterialBindingAPI.Apply(prim).Bind(gm, UsdShade.Tokens.weakerThanDescendants, "physics")
    ...
    try:
        ...
        _contact_ok = False
        print(f"[probe] WARN: contact report 購読不可 → 接触力は測れず z のみで判定: {_e}", flush=True)
    ...
    for mu in MU_DEFAULTS:
        ...
        for dmm in DIAMS_MM:
            ...
            print(f"[probe] μ={mu} D={dmm}mm → {'✅HELD' if held else '❌SLIP'} "
                  ...
                  f"finger={fpos}", flush=True)
            stage.RemovePrim(path)

    print("\n[probe] ===== 摩擦把持プローブ結果表 =====", flush=True)
    print(f"[probe] 支えるべき重量 = {weight:.2f} N (質量 {OBJ_MASS*1000:.0f}g)"
          + ("" if _contact_ok else "  ※contact report 不可＝力は0表示、z のみ有効"), flush=True)
    ...
    for mu, dmm, held, dz, n_clamp, f_fric, margin, f_normal in results:
        print(f"[probe]  {mu:<4} {dmm:<4} {'HELD' if held else 'SLIP'}  {dz*1000:+6.1f}  "
              f"{n_clamp:6.2f}N  {mu*n_clamp:7.2f}N     ×{margin:<4.1f} {f_fric:6.2f}N", flush=True)
    n_held = sum(1 for r in results if r[2])
    print(f"[probe] 保持成立 {n_held}/{len(results)}  "
          f"（μ×N_clamp ≥ 重量 が摩擦で掴める条件。z 保持と接触継続の両立で HELD 判定）", flush=True)
