...
OUTDIR = os.getenv("SIM_CAM_OUTDIR", "/tmp/claude-1000/-home-kota-ueda-Desktop-dimos-hackathon/1487315e-e890-48a9-8754-b1a3f80b5f18/scratchpad/sim_out")
...
while time.time() - t0 < SECS:
    ...
    if n % 30 == 0 and last_depth is not None:
        ...
        if valid.size:
            print(f"[cam-sub] frame {n}: depth m min/med/max = {valid.min():.2f}/{np.median(valid):.2f}/{valid.max():.2f}", flush=True)

os.makedirs(OUTDIR, exist_ok=True)
if last_bgr is not None:
    cv2.imwrite(f"{OUTDIR}/cam_color.png", last_bgr)
if last_depth is not None:
    vis = np.clip(last_depth / max(0.1, float(np.percentile(last_depth[last_depth > 0], 95) or 1)) * 255, 0, 255).astype(np.uint8)
