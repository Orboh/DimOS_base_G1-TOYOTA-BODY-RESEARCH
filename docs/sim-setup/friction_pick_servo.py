...
from walk_approach_pick import BridgeLog, X_TIP, Z_COMP, X_CMD_MAX
...
def main() -> int:
    ...
    print(f"[servo] reach rest={tuple(round(v,3) for v in rest)} → cmd={tuple(round(float(v),3) for v in tgt)}",
          flush=True)
    ...
    for it in range(int(os.getenv("SERVO_ITERS", "6"))):
        ...
        if d is None:
            ...
            hold(0.6, gq=0.0)
            continue
        print(f"[servo] it{it}: Δ(okra-gap)={tuple(round(v,3) for v in d)} cmd={tuple(round(float(v),3) for v in tgt)}",
              flush=True)
    ...
    okz = log.okra_z_max()
    grasped = okz > 0.82
    print(f"[servo] === lift 後 okra_z(max)={okz:.3f} → {'✅ 把持成立（摩擦で保持）' if grasped else '❌ 未把持（滑落）'} ===",
          flush=True)
