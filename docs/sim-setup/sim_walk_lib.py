...
def make_floating_base(
    ...
    for p in Usd.PrimRange(g1_prim):
        if p.GetName() in CANON_G1_29:
            ...
            n_arm += 1
    print(
        f"[walklib] floating base 化: root={pelvis_path} solver={solver_pos}/{solver_vel} "
        f"selfColl={bool(self_collision)} armature={armature}×{n_arm}関節",
        flush=True,
    )
...
def apply_sdk_gains(
    robot, dp: dict, isaac_idx_for_motor: list[int | None], kp_scale: float = 1.0
) -> None:
    ...
    print(
        f"[walklib] SDK順 PD gains 適用 kp_scale={kp_scale}"
        f"（knee kp={kp[knee]:.0f} kd={kd[knee]:.1f}）",
        flush=True,
    )
...
class PolicyWalker:
    ...
    def place_upright(
        ...
        robot.set_world_pose(
            position=np.array([xy[0], xy[1], base_z], dtype=np.float32),
        ...
        print(
            f"[walklib] 直立配置+settle{settle}: pelvis_z={float(np.asarray(p).reshape(-1)[2]):.3f} "
            f"quat_w={float(np.asarray(q).reshape(-1)[0]):.3f}",
            flush=True,
        )
    ...
    def tick(self, robot, cmd, mask_motors: list[int] | None = None) -> np.ndarray:
        ...
        terms = {
            "ang": (w * dp["ang_scale"]).astype(np.float32),
            "grav": g.astype(np.float32),
        ...
        for k in self._TERMS:
            if not self._hist[k]:  # 初回: 履歴を同値で埋める（C++ reset() と同じ）
                ...
                self._hist[k].append(terms[k].copy())
        obs = np.concatenate([np.concatenate(list(self._hist[k])) for k in self._TERMS]).astype(
            np.float32
        )[None, :]
