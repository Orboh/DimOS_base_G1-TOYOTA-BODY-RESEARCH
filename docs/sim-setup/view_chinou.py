...
def main() -> None:
    ...
    room_prim = stage.GetPrimAtPath("/World/ChinouCenter") or stage.GetPrimAtPath(
        "/World/OkraField"
    )
    if not (room_prim and room_prim.IsValid()):
        room_prim = next(
            (p for p in stage.TraverseAll() if p.IsA(UsdGeom.Mesh)), None
    ...
    print(
        f"[chinou] ROOM world size (m) = {rsz}  min={tuple(round(v, 2) for v in rmn)} max={tuple(round(v, 2) for v in rmx)}",
        flush=True,
    )
    ...
    if args.ceil_lights > 0:
        ...
        ncl = sim_scene.add_ceiling_lights(
            stage, n=args.ceil_lights, intensity=args.ceil_intensity, radius=args.ceil_radius
        )
        ...
        if args.okra > 0:
            ...
            for r in range(rows):
                for c in range(cols):
                    ...
                    k += 1
            print(
                f"[chinou] okra x{k} を机上に直立固定（world剛ジョイント）。引張8Nで収穫可",
                flush=True,
            )
    ...
    try:
        robot.set_world_pose(
            position=np.array([0.0, 0.0, lift]), orientation=np.array([1.0, 0.0, 0.0, 0.0])
        )
    except Exception as e:
