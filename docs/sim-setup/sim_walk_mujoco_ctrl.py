...
def main() -> int:
    ...
    if not state["got"]:
        print(
            "[walk-mj] ❌ rt/lowstate が来ない。unitree_mujoco(ROBOT=g1,DOMAIN_ID=1) 起動中か確認",
            flush=True,
        )
        return 1
    ...
    kp = np.zeros(29, np.float32)
    kd = np.zeros(29, np.float32)
    default_motor = np.zeros(29, np.float32)
    for i in range(29):
        kp[jmap[i]] = stiffness[i]
        kd[jmap[i]] = damping[i]
    ...
    while time.time() - t0 < secs:
        ...
        step_vec = np.concatenate(
            [
                state["gyro"] * ang_scale,
                proj_gravity(state["quat"]),
                cmd,
                jpos_rel,
                jvel_rel,
                last_action,
        ...
        if step % 50 == 0:
            print(
                f"[walk-mj] t={time.time() - t0:.1f}s mode={state['mode_machine']} "
                f"gz={state['gyro'][2]:+.2f}",
                flush=True,
            )
        time.sleep(step_dt)
