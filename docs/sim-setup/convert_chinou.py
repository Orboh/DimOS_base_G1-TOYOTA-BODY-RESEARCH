...
for o in objs:
    ...
    allV.append(V); allF.append(F + voff); allC.append(C)
    voff += len(V)
...
if TARGET_TRIS and len(F) > TARGET_TRIS:
    ...
    try:
        ...
        Vd = np.asarray(Vd, dtype=np.float64); Fd = np.asarray(Fd)
        log(f"fast-simplification done: V={len(Vd):,} F={len(Fd):,}")
    except Exception as e:
        log(f"fast-simplification FAILED ({type(e).__name__}: {e}); fallback open3d vertex-clustering")
        ...
        Vd = np.asarray(m.vertices); Fd = np.asarray(m.triangles)
...
if up != 2:
    ...
    order = [0,1,2]; order[2], order[up] = order[up], order[2]
    V = V[:, order]
...
mn, mx = V.min(0), V.max(0)
mesh_p.CreateExtentAttr([Gf.Vec3f(float(mn[0]), float(mn[1]), float(mn[2])),
