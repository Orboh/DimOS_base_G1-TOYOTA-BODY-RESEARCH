...
def grab_frame(host: str = "127.0.0.1", port: int = 5555, topic: str = "ego_view",
               secs: float = 3.0):
...
def detect_okra_torso(host: str = "127.0.0.1", port: int = 5555, topic: str = "ego_view",
                      *, weight: str = DEFAULT_WEIGHT, conf: float = 0.25,
                      secs: float = 3.0, save_dir: str | None = None) -> list[YoloDet]:
    ...
    for i in range(nd):
        ...
        out.append(YoloDet(torso=(rp[0] + t[0], rp[1] + t[1], rp[2] + t[2]),
                           conf=cf, uv=(u, v_), depth_m=d))
    out.sort(key=lambda d: -d.conf)
    return out


__all__ = ["YoloDet", "grab_frame", "detect_okra_torso", "DEFAULT_WEIGHT"]
