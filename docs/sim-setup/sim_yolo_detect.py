...
OUT = os.getenv(
    "SIM_YOLO_OUT",
    "/tmp/claude-1000/-home-kota-ueda-Desktop-dimos-hackathon/ff3f8d27-e694-4585-88d3-beb6daaeb929/scratchpad/sim_yolo",
)
...
def main() -> int:
    ...
    print(
        f"[yolo] received {n} frames, color={bgr.shape}, depth={'yes' if depth is not None else 'no'}, K={K}",
        flush=True,
    )
    ...
    print(
        f"[yolo] weight={os.path.basename(WEIGHT)} conf>={CONF} → 検出 {nd} 個 names={names}",
        flush=True,
    )
    if nd == 0:
        print(
            "[yolo] 検出0（sim画像はOOD＝想定内）。配管(camera→YOLO→画像)は通った。注釈画像を保存。",
            flush=True,
        )
    ...
    for i in range(nd):
        ...
        print(
            f"  det[{i}] {names.get(cls, cls)} conf={conf:.2f} uv=({u:.0f},{v_:.0f}) "
            f"depth={d:.2f}m torso=({p_tor[0]:.3f},{p_tor[1]:.3f},{p_tor[2]:.3f})",
            flush=True,
        )
