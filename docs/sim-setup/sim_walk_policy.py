...
def main() -> int:
    ...
    print(
        f"[walk] policy obs hist={dp['hist']} step_dt={dp['step_dt']} cmd=({vx},{vy},{wz})",
        flush=True,
    )
    ...
    while _t.time() - t0 < secs and sim_app.is_running():
        ...
        if step < 5:  # 立ち上がり確認: grav≈[0,0,-1] / angvel≈0 / |action| 有界なら健全
            d = walker.dbg
            print(
                f"[diag] step{step - 1}: grav={np.round(d['grav'], 2).tolist()} "
                f"|angvel|={float(np.linalg.norm(d['angvel'])):.2f} "
                f"|action|max={float(np.abs(d['action']).max()):.2f}",
                flush=True,
            )
        if step % 25 == 0:
            ...
            print(
                f"[walk] t={_t.time() - t0:.1f}s pos=({px:+.2f},{py:+.2f},{pz:.3f})"
                f"（立=z~0.79, 転=低z）",
                flush=True,
            )
