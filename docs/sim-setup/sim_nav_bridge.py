...
def main() -> None:
    ...
    print(
        f"[nav-bridge] lidar rays/scan = {len(dirs)} (elevs={LIDAR_ELEVS_DEG}, az_step={LIDAR_AZ_STEP})",
        flush=True,
    )
    ...
    while True:
        ...
        try:
            robot.set_world_pose(
                position=np.array([x, y, lift]), orientation=np.array([qw, 0.0, 0.0, qz])
            )  # (w,x,y,z)
        except Exception as _e:
            if step % 300 == 0:
        ...
        if step % LIDAR_EVERY == 0:
            ...
            if step % (LIDAR_EVERY * 60) == 0:
                print(
                    f"[nav-bridge] step={step} pose=({x:.2f},{y:.2f},{math.degrees(yaw):.0f}deg) "
                    f"scan_pts={n} cmd=({cmd['vx']:.2f},{cmd['vy']:.2f},{cmd['wz']:.2f})",
                    flush=True,
                )
