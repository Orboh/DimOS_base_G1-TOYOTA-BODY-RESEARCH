...
FixedCuboid("/World/Floor", position=np.array([0, 0, -0.05]), scale=np.array([2.0, 2.0, 0.1]),
...
# 対照の弾性ボール（e=0.9）
bouncy = PhysicsMaterial("/World/PM/bouncy", static_friction=0.5, dynamic_friction=0.5, restitution=0.9)
...
ball = DynamicSphere("/Ball", position=np.array([0.2, 0, DROP_Z]), radius=0.03, mass=0.05,
                     physics_material=bouncy)
# 摩擦テスト用オクラ（水平速度を与える）
...
zr = np.array(z_real); zb = np.array(z_ball)
...
if ARGS.gui:
    ...
    DROP_POS = {okra_real: [0.0, 0.0, DROP_Z], ball: [0.2, 0.0, DROP_Z], okra_slide: [-0.4, 0.0, 0.01]}
    period, k = 260, 0
    while app.is_running():
        ...
        if ph == 0:  # 周期的に再投下
            for prim, pos in DROP_POS.items():
                ...
                try:
                    prim.set_linear_velocity(np.zeros(3)); prim.set_angular_velocity(np.zeros(3))
                except Exception:  # noqa: BLE001
                    pass
