...
def gripper_lines(transform: np.ndarray):
    ...
    segs = [(world[0], world[1]), (world[1], world[2]), (world[1], world[3]),
            (world[2], world[4]), (world[3], world[5])]
    return segs
...
def main() -> int:
    ...
    for i, (name, elev, azim) in enumerate(views):
        ...
        allp = np.vstack([pts] + [np.array(s).reshape(-1, 3) for g in grasps for s in gripper_lines(g)])
        c = allp.mean(axis=0)
        r = (allp.max(axis=0) - allp.min(axis=0)).max() / 2.0 + 1e-3
        ax.set_xlim(c[0] - r, c[0] + r); ax.set_ylim(c[1] - r, c[1] + r); ax.set_zlim(c[2] - r, c[2] + r)
    fig.suptitle(f"{args.title}  (top{len(grasps)}/{len(data['grasps'])}, "
                 f"score {min(scores):.2f}-{max(scores):.2f})" if scores else args.title)
    fig.tight_layout()
