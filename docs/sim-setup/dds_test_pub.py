...
def build_cfg() -> str:
    ...
    tr = (f'<Tracing><Verbosity>config</Verbosity><OutputFile>{trace}</OutputFile></Tracing>' if trace else "")
    if peers:
        peers_xml = "<Peers>" + "".join(f'<Peer address="{p}"/>' for p in peers) + "</Peers>"
        return (f'<?xml version="1.0" encoding="UTF-8"?><CycloneDDS xmlns="https://cdds.io/config">'
...
def main() -> None:
    ...
    print(f"[pub] publishing rt/arm_sdk @50Hz for {secs}s (iface={os.getenv('SIM_DDS_IFACE','wlp0s20f3')})", flush=True)
    n = int(secs * 50)
    for i in range(n):
        ...
        w.write(lc)
        if i % 50 == 0:
            print(f"[pub] t={i/50:.1f}s r_shoulder_pitch={lc.motor_cmd[R_SHOULDER_PITCH].q:.3f} "
                  f"r_elbow={lc.motor_cmd[R_ELBOW].q:.3f}", flush=True)
        time.sleep(0.02)
