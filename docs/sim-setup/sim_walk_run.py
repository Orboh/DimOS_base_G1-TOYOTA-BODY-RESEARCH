...
import config           # noqa: E402  (unitree_mujoco/simulate_python/config.py)
...
def main() -> int:
    ...
    viewer = mujoco.viewer.launch_passive(
        mj_model, mj_data, key_callback=band.MujuocoKeyCallback
    )
    time.sleep(0.2)
    ...
    print(f"[walk-sim] DDS domain={config.DOMAIN_ID} iface={config.INTERFACE} "
          f"band_link=torso_link len0={band.length} joy_file={bridge._virtual_remote_path}",
          flush=True)
    print("[walk-sim] g1_ctrl と sim_walk_joy.py を起動してください（band は /tmp/sim_band.txt でも制御可）",
          flush=True)
    ...
    try:
        ...
        while viewer.is_running():
            time.sleep(1.0)
            x, y, z = (float(mj_data.qpos[0]), float(mj_data.qpos[1]),
                       float(mj_data.qpos[2]))
            fell = z < 0.45  # 転倒判定（立位 base≈0.78m）
            print(f"[walk-sim] t={time.time()-t0:4.0f}s base=({x:+.2f},{y:+.2f},{z:+.2f}) "
                  f"band.enable={band.enable} len={band.length:.2f}"
                  f"{'  ⚠FALL' if fell else ''}", flush=True)
