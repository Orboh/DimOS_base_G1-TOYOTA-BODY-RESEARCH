...
class BridgeLog:
    ...
    def delta_for(self, idx: int) -> tuple[float, float, float] | None:
        ...
        pat = (
            rf"nearest=Okra_{idx} okra_z=[0-9.]+ dist=[0-9.]+.*?"
            r"Δ\(okra-gap\)=\((-?[0-9.]+), (-?[0-9.]+), (-?[0-9.]+)\)"
        )
...
def main() -> int:
    ...
    if os.getenv("AP_PRERAISE", "1") == "1":
        pre = np.array(
            [float(x) for x in os.getenv("AP_PRERAISE_POSE", "0.40,-0.16,0.20").split(",")]
        )
        r0 = ik.solve(pre, [0.0] * 29)
        if r0 is not None:
            print(
                f"[ap] Phase 0: 事前挙手 {tuple(float(v) for v in pre)}（机より上・err={r0.err:.4f}）",
                flush=True,
            )
    ...
    print(
        f"[ap] Phase B: リーチ rest={tuple(round(v, 3) for v in rest)} → cmd={tuple(round(float(v), 3) for v in tgt)}",
        flush=True,
    )
    ...
    grasped = okz > 0.82
    print(
        f"[ap]   lift 後 okra_z(max)={okz:.3f} → {'✅ 把持成立' if grasped else '❌ 未把持'}",
        flush=True,
    )
