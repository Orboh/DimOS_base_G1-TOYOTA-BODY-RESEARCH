...
def main() -> int:
    ...
    if res is None:
        print(
            f"[ik-pub] IK 解けず target(torso)={tor} → 中止（reach 内のオクラを指定して）",
            flush=True,
        )
        return 1
    arm14 = [float(x) for x in res.arm14]
    print(
        f"[ik-pub] IK OK: err={res.err:.4f}m converged={res.converged} target_torso={tor}",
        flush=True,
    )
    ...
    pub.Init()
    print(
        f"[ik-pub] rt/arm_sdk up (iface={iface}); ramp {ramp_s}s → hold（total {secs}s）",
        flush=True,
    )
