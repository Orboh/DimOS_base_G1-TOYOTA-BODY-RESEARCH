...
def setup_room() -> None:
    ...
    for p in s.TraverseAll():
        if p.HasAPI(UsdPhysics.CollisionAPI) and p.GetPath().pathString.startswith("/World/Colliders/"):
            bind_physics(p, mat); n += 1
    s.GetRootLayer().Save()
    print(f"[room] floor_mat を {n} collider に割当 (static={FLOOR['static']} dyn={FLOOR['dynamic']} rest={FLOOR['restitution']})")
...
def setup_robot() -> None:
    ...
    for p in s.TraverseAll():
        ...
        if "ankle_roll" in path:
            bind_physics(p, foot_mat); nf += 1
        elif "right_hand" in path:  # Dex1 右手（base + 指リンク）
            bind_physics(p, grip_mat); ng += 1
    s.GetRootLayer().Save()
    print(f"[robot] foot_mat→{nf}（足裏） grip_mat→{ng}（Dex1指）割当 "
