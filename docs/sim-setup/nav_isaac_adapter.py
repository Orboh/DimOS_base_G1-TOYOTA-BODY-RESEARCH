...
class NavIsaacAdapter:
    ...
    def _on_lidar(self, _channel, data) -> None:
        ...
        if n <= 0 or len(data) < need:
            return
        pts = (
            np.frombuffer(data, dtype="<f4", count=n * 3, offset=4).reshape(n, 3).astype(np.float64)
        )
    ...
    def spin(self) -> None:
        print(
            "[adapter] up. isaac/lidar,isaac/odom → /registered_scan,/odometry,/tf ; /cmd_vel → isaac/cmd_vel",
            flush=True,
        )
        last = 0.0
        while True:
            ...
            if t - last > 2.0:
                print(
                    f"[adapter] scan={self.n_scan} odom={self.n_odom} cmd={self.n_cmd}", flush=True
                )
                last = t
