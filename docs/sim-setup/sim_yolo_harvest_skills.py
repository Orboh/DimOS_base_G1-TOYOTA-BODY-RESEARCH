...
_Q_CLOSE = 4.4
_BASE_SPEED = float(
    os.getenv("SIM_BASE_SPEED", "0.30")
)  # [m/s] base 移動速度（LocoClient 風, SS-07）
_BASE_RATE_HZ = float(
    os.getenv("SIM_BASE_RATE_HZ", "10")
)  # cmd_vel ストリーム周波数（watchdog 対策）
...
class SimYoloHarvestSkills:
    ...
    def __init__(
        ...
        *,
        ...
        cam_host: str = "127.0.0.1",
        cam_port: int = 5555,
        conf: float = 0.25,
        dedup_m: float = 0.06,
        save_dir: str | None = None,
        basket_torso: tuple[float, float, float] = (0.23, 0.011, -0.072),
    ) -> None:
        ...
        self._skipped_pos: list[
            np.ndarray
        ] = []  # cut_ok ゲートで見送ったオクラ（再試行ループ防止）
        ...
        if self._vlm:
            ...
            print(
                f"[yolo-skills] VLM=ON host={self._vlm_host} model={self._vlm_model} "
                f"verify={self._verify_mode} reachable={reachable}",
                flush=True,
            )
            if not reachable:
                print(
                    "[yolo-skills] ⚠️ ollama 未到達: cut_ok は安全側 False＝切らない（picks=0 になる）。"
                    "tailscale と Jetson moondream を確認。VLM 無効化は SIM_VLM=0",
                    flush=True,
                )
        else:
            self._cut_ok = None
            ...
            print(
                "[yolo-skills] VLM=OFF（SIM_VLM=0）: cut_ok ゲート無し・verify=True 固定（従来挙動）",
                flush=True,
            )
        ...
        print(
            f"[yolo-skills] DDS up iface={iface} peers={peers or 'mcast'} (warmup); "
            f"cam={cam_host}:{cam_port} conf>={conf}; F-07=記録固定角(body_center→drop)",
            flush=True,
        )
    ...
    def _ramp(self, to14, secs, grip_q, weight_to=1.0):
        ...
        for i in range(n):
            s = (i + 1) / n
            self._send(
                [frm[j] + (to14[j] - frm[j]) * s for j in range(14)],
                w0 + (weight_to - w0) * s,
                grip_q,
            )
            time.sleep(0.02)
    ...
    def detect_okra(self) -> list[Okra]:
        ...
        for i, d in enumerate(dets):
            ...
            if any(
                np.linalg.norm(p - q) < self._dedup_m
                for q in (*self._picked_pos, *self._skipped_pos)
            ):
                continue
            out.append(
                Okra(
                    id=f"y{i}_{d.conf:.2f}",
    ...
    def grasp_okra(self, okra: Okra, force: float) -> None:
        ...
        p = np.array(
            [okra.pos_3d["x"], okra.pos_3d["y"], okra.pos_3d["z"]], dtype=float
        )  # graph 系
        ...
        if res is None:
            ...
            return
        print(
            f"[yolo-skills] GRASP {okra.id} graph={np.round(p, 3)} ik={np.round(p_ik, 3)} err={res.err:.4f}",
            flush=True,
        )
        ...
        if self._cut_ok is not None:
            ...
            if not ok:
                print(
                    f"[yolo-skills] {okra.id} → 把持中止（安全側・このオクラは見送り）", flush=True
                )
        ...
        r_lift = self._ik.solve(
            p_ik + np.array([-0.05, 0.0, 0.18]), [0.0] * 29
        )  # lift（IK系: 手前へ引き＋上）
        ...
        print("[yolo-skills] F-07 籠収納 OK（body_center→drop 固定角再生）", flush=True)
    ...
    def relative_move(self, lateral: float, forward: float = 0.0, yaw: float = 0.0) -> None:
        ...
        print(
            f"[yolo-skills] relative_move(lat={lateral:+.2f},fwd={forward:+.2f}) "
            f"→ cmd_vel({vx:+.2f},{vy:+.2f}) {dur:.1f}s → base world=({self._base_xy[0]:+.2f},{self._base_xy[1]:+.2f})",
            flush=True,
        )
        time.sleep(0.3)  # 整定待ち
