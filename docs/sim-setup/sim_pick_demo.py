...
def main() -> int:
    ...
    print(
        f"[pick] reach IK err={r_reach.err:.4f}; "
        f"lift {'IK err=' + format(r_lift.err, '.4f') if r_lift else 'IK失敗→reach保持'}",
        flush=True,
    )
    ...
    grip_pub.Init()
    print(
        f"[pick] rt/arm_sdk + rt/dex1/right/cmd up (iface={iface}, peers={peers or 'mcast'})",
        flush=True,
    )
