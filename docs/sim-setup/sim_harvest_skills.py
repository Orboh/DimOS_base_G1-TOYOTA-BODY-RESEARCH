...
class SimHarvestSkills:
    ...
    def __init__(
        ...
        *,
        ...
        for _ in range(80):
            ...
            time.sleep(0.02)
        print(
            f"[sim-skills] DDS up iface={iface} peers={peers or 'mcast'} (warmup done); okra={sorted(int(k) for k in self._okra)}",
            flush=True,
        )
        ...
        print(
            f"[sim-skills] F-07 籠位置(torso)={tuple(round(v, 3) for v in self._basket_torso)}"
            " （SIM_BASKET_TORSO で上書き可）",
            flush=True,
        )
    ...
    def grasp_okra(self, okra: Okra, force: float) -> None:
        ...
        print(
            f"[sim-skills] GRASP okra{okra.id} (idx={idx}) reach IK err={res.err:.4f}", flush=True
        )
        ...
        ok = self._place()
        print(
            f"[sim-skills] F-07 籠収納 {'OK' if ok else 'FAIL（IK解けず・要 SIM_BASKET_TORSO 調整）'}",
            flush=True,
        )
    ...
    def relative_move(self, lateral: float, forward: float = 0.0, yaw: float = 0.0) -> None:
        print(
            f"[sim-skills] relative_move(lat={lateral:.2f},fwd={forward:.2f}) — sim では no-op",
            flush=True,
        )
