...
for p in stage.GetPseudoRoot().GetChildren():
    try:
        mn, mx, s = bb(p)
        print(
            f"  /{p.GetName():18s} [{p.GetTypeName()}] size=({s[0]:.2f},{s[1]:.2f},{s[2]:.2f}) y=[{mn[1]:.2f},{mx[1]:.2f}] z=[{mn[2]:.2f},{mx[2]:.2f}]",
            flush=True,
        )
...
print(
    f"\n=== FULL stage bbox: size=({s[0]:.2f},{s[1]:.2f},{s[2]:.2f}) min={tuple(round(v, 2) for v in mn)} max={tuple(round(v, 2) for v in mx)} ===",
    flush=True,
)
