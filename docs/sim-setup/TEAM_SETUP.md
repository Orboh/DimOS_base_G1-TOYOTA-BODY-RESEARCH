...
Isaac Sim 環境を再現するための通し手順。詳細・つまずき集は [`sim-setup-notes.md`](/docs/sim-setup-notes.md) を参照。
...
| `okra_field_full.usd` | 上記の**間引きなしフル解像度版**（63M面 / 1.15GB）。⚠️ **8GB機では開けない・16GB+ GPU機用**。通常は 2M 版を使う。詳細 [`OKRA_FIELD.md`](/docs/sim-setup/OKRA_FIELD.md#L68) |
...
- 詳細・既知の落とし穴（user-site 汚染 / room スケール / DDS over tailscale 等）は [`sim-setup-notes.md`](/docs/sim-setup-notes.md)。
