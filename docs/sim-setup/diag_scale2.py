...
def native_info(path):
    ...
    return (
        UsdGeom.GetStageMetersPerUnit(s),
        str(UsdGeom.GetStageUpAxis(s)),
        (s.GetDefaultPrim().GetPath() if s.GetDefaultPrim() else None),
    )
...
for nm, pp in (("Room", "/World/Room"), ("G1", "/World/G1")):
    mn, mx, sz = wbb(pp)
    print(
        f"  {nm}: size=({sz[0]:.3f}, {sz[1]:.3f}, {sz[2]:.3f})  min={tuple(round(v, 2) for v in mn)} max={tuple(round(v, 2) for v in mx)}",
        flush=True,
    )
