...
def main() -> int:
    ...
    xs = [
        cfg.reach.x_min + (cfg.reach.x_max - cfg.reach.x_min) * i / (N_OKRA - 1)
        for i in range(N_OKRA)
    ]
    field = [
        FieldOkra(f"o{i}", x=round(x, 3), y=0.45, z=0.80, ripeness=0.95) for i, x in enumerate(xs)
    ]
