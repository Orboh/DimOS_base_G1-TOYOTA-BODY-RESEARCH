...
def gripper_const_from_policy(policy):
    ...
    try:
        ...
        print(
            f"gripper_width: training mean={mean:.6f} range=[{lo:.6f}, {hi:.6f}] "
            f"(was hard-coded {_GRIPPER_FALLBACK})",
            flush=True,
        )
        return mean
    except Exception as e:
        print(
            f"gripper_width: could not read normalizer ({e!r}); "
            f"falling back to {_GRIPPER_FALLBACK}",
            flush=True,
        )
    ...
    def _run(self):
        ...
        while not self._stop.is_set():
            ...
            if (
                ...
                and now <= self._rec_active_until
                ...
                try:
                    ...
                    _cv2.imwrite(
                        os.path.join(self._rec_dir, f"cam_{self._rec_tag}_{self._rec_n:06d}.png"),
                    ...
                    self._rec_index.write(
                        json.dumps({"n": self._rec_n, "t": now, "tag": self._rec_tag}) + "\n"
                    )
                    ...
                    if not self._rec_warned:
                        ...
                        print(
                            f"frame recording failed ({exc!r}); capture continues, "
                            "the analysis video will be short",
                            flush=True,
                        )
        if cap is not None:
            cap.release()
...
class PolicyServer:
    def __init__(
        ...
        self._record_n = 0
        self._gripper_const = (
            gripper_override if gripper_override is not None else gripper_const_from_policy(policy)
        )
    ...
    def camera_health(self):
        ...
        if cam_age_ms > self._max_cam_age_ms:
            h["ok"] = False
            h["reason"] = (
                f"camera STALE: newest frame is {cam_age_ms / 1000.0:.1f}s old "
            ...
            h["ok"] = False
            h["reason"] = (
                f"camera SIGNAL DEAD: {black_frac * 100:.1f}% of the frame is black "
    ...
    def predict(self, t_now, pos, aa):
        ...
        if self._record_dir is not None:
            ...
            _cv2.imwrite(
                os.path.join(self._record_dir, f"frame_{self._record_n:06d}.png"),
        ...
        cur = pose_to_mat(
            np.concatenate(
                [env_obs["robot0_eef_pos"][-1], env_obs["robot0_eef_rot_axis_angle"][-1]], axis=-1
            )
        )
        action_mat = convert_pose_mat_rep(
            pose10d_to_mat(raw[..., :9]),
            base_pose_mat=cur,
            pose_rep=self._action_pose_repr,
            backward=True,
        )
...
@click.option(
    "--dummy-cam", is_flag=True, default=False, help="no GoPro: feed zeros (offline IPC test)"
)
...
@click.option(
    "--log-every", default=1, type=int, help="print a diagnostic line every N requests (0 = never)"
)
...
def main(
    ...
    preproc = build_preproc(
        fisheye_converter, no_mirror, out_res=out_res, gripper_mask=gripper_mask
    )
    print(
        "gripper mask: {} ({})".format(
            "ON" if gripper_mask else "OFF",
            "stock UMI datasets" if gripper_mask else "roboharvest datasets",
        ),
        flush=True,
    )
    cam = CameraThread(
    ...
    print(
        "camera guards: max_age={:.0f}ms max_black={:.0f}% enabled={}{}".format(
            max_cam_age_ms,
            max_black_frac * 100,
            not no_camera_check,
            "  (black guard OFF: --dummy-cam)" if dummy_cam else "",
        ),
        flush=True,
    )
    ...
    sock.bind(addr)
    print(
        f"umi_policy_server ready on {addr} (dummy_cam={dummy_cam}, log_every={log_every}, "
        f"trace={trace or 'OFF'}). Ctrl-C to stop.",
        flush=True,
    )
    n_req = 0
    try:
            ...
            try:
                ...
                if req.get("cmd") == "health":
                    ...
                    if not quiet:
                        age = h["cam_age_ms"]
                        blk = h["black_frac"]
                        age_s = "-" if age is None else f"{age:.0f}ms"
                        blk_s = "-" if blk is None else f"{blk * 100:.1f}%"
                        print(
                            "HEALTH req#{} ok={} cam_age={} black={} {}".format(
                                n_req, h["ok"], age_s, blk_s, h["reason"]
                            ),
                            flush=True,
                        )
                ...
                if req.get("reset"):
                    ...
                    if not quiet:
                        print(
                            f"RESET req#{n_req} episode_start pos[{f3(req['eef_pos'])}] "
                            f"aa[{f3(req['eef_rot_aa'])}]",
                            flush=True,
                        )
                t0 = time.time()
                ...
                rep = {
                    "ok": True,
                    "actions": [[float(x) for x in row] for row in acts],
                    "n": int(acts.shape[0]),
                    "infer_ms": infer_ms,
