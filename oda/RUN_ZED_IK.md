...
1. 屋内(ネットあり)で一度 `dimos run unitree-g1-zed-ik-view` を起動し、点群が出るのを確認
   しておく(=キャッシュが温まっている証明。ZED個体を替えた場合はこのステップ必須 —
   キャリブファイルはシリアル番号ごとに1回DLされる)
2. **別のPC**(AGX Orin等)で初めて動かす場合も同様に、屋内で初回起動を済ませておく
3. `sudo`のネットワーク設定2行(手順1)は再起動ごとに必要だが、オフラインで問題なく実行可
4. NTP同期は不要(タイムスタンプは全てローカル時計基準で、鮮度チェックも同一マシン内比較)
