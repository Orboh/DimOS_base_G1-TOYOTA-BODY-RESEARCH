...
#
# IK_ABOVE_M: 3段階アプローチ[m]。0=無効。
#   (1)現在のx,yのまま真上へ上げる (2)目標へ水平移動 (3)真下に降りる
#   水平移動する高さ = 目標z + IK_ABOVE_M。
#   **0.001 が推奨(2026-09-04 実機確認)**: 水平移動の高さが目標zとほぼ一致するので
#   「指定座標の高さまで手を上げる -> 水平に接近する」になり、(3)は0.1cmで実質消える。
#   0.10 だと目標の10cm上を通って10cm降ろすが、この10cmは「腕を上げる途中で株に
#   当たる」問題への対症療法(2026-07-21)で決まった値であり、収穫タスク上の根拠はない。
#   重心から茎(切断点)へ降ろす動作は本来これとは別の、作物形状に基づく工程。
...
FRONT="${IK_FRONT_M:-0.0}"; ABOVE="${IK_ABOVE_M:-0.001}"
