...
def main() -> None:
    ...
    for det in dets:
        ...
        if b2 is not None:
            c = b2.center
            print(
                f"  [B2 bbox+filter ] optical(x={c.x:+.3f}, y={c.y:+.3f}, z={c.z:+.3f}) m  "
        ...
        print(
            f"  [C mask         ] optical(x={c_med[0]:+.3f}, y={c_med[1]:+.3f}, z={c_med[2]:+.3f}) m  "
        ...
        u_obj = uu[sel]
        v_obj = vv[sel]
        for pu, pv in zip(u_obj, v_obj, strict=False):
            bgr[pv, pu] = (255, 255, 0)
        ...
        cv2.putText(
            bgr,
            f"{det.name} mask z={c_med[2]:.2f}m",
            (x1, max(0, y1 - 8)),
